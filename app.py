...
st.write("The Application provides Bone Fracture Detection using multiple state-of-the-art computer vision models such as Yolo V10,Yolo V11,Vgg 16 and ResNet.")
...
with tab1:
   ...
   st.image(v11, caption="Yolov11 Architecture", width=None, use_column_width=True)
...
with tab2:
    ...
    if st.session_state.clicked:
        ...
        if image is not None:
            ...
            # Check if YOLOv10 model is selected
            if model == "yolov10":
                print("Inside new yolov10 function")
                try:
                    yolo_detection_model = YOLO(yolo_path_10_new)
                    yolo_detection_model.load()
                except Exception as ex:
                    st.error(f"Unable to load model. Check the specified path: {yolo_path_10_new}")
                    st.error(ex)
                ...
                # Column 1: Upload and display the image
                with col1:                    
                    if image:
                        uploaded_image = PIL.Image.open(image)  # Load the uploaded image
                        ...
                        # Check if "Execution" button is clicked
                        with st.spinner("Running..."):
                                # Perform prediction using the YOLOv10 model
                                yolo_detection_model_new = YOLO(yolo_path_10_new)
                                res = yolo_detection_model_new(uploaded_image)
                                detection_image = res[0].plot()
                                save_path = r"C:\Users\kesav_u5svehq\Downloads\bone_fracture_detection_main\testing_images\manu.jpg"
                                import cv2
                ...
                try:
                    ...
                    st.error(f"Unable to load model. Check the specified path: {yolo_path_v11}")
                    st.error(ex)
                        ...
                        if st.button("Execution"):
                            with st.spinner("Running..."):
                                res = yolo_detection_model.predict(uploaded_image,
                                                    conf=conf_threshold, augment=True, max_det=2)
                                boxes = res[0].boxes
